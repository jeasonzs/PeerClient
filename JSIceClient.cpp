...
void log_func(int level, const char *data, int len)
{
    ...
    if (log_fhnd) {
        if (fwrite(data, len, 1, log_fhnd) != 1)
        return;
    }
}
...
//TODO:setter
void JSIceClient::setIceCb(pj_ice_strans_cb *iceCb)
{
    memcpy(&this->iceCb,iceCb,sizeof(pj_ice_strans_cb));
}


void JSIceClient::setLogFile(char* logFile)
{
...
//TODO:outside operation
/*
 * Create ICE stream transport instance, invoked from the menu.
 */
void JSIceClient::icedemo_create_instance(void)
{
 ...
 */
void JSIceClient::icedemo_destroy_instance(void)
{
    if (icest == NULL) {
 ...
 */
void JSIceClient::icedemo_init_session(unsigned rolechar)
{
 ...
 */
void JSIceClient::icedemo_stop_session(void)
{
    pj_status_t status;
 ...
 */
void JSIceClient::icedemo_input_remote(void)
{
    char linebuf[80];
 ...
 */
void JSIceClient::icedemo_start_nego(void)
{
 ...
 */
void JSIceClient::icedemo_send_data(unsigned comp_id, const char *data)
{
    pj_status_t status;
...
//TODO:inside operation
void JSIceClient::threadWork()
{
    printf("thread work\n");
 ...
 */
int JSIceClient::icedemo_worker_thread(void *context)
{
    JSIceClient* iceClient = (JSIceClient*)context;
    
    iceClient->threadWork();
    
    return 0;
}

/* Utility to display error messages */
void JSIceClient::icedemo_perror(const char *title, pj_status_t status)
{
    char errmsg[PJ_ERR_MSG_SIZE];
 ...
 */
void JSIceClient::err_exit(const char *title, pj_status_t status)
{
    if (status != PJ_SUCCESS) {
 ...
 */
pj_status_t JSIceClient::handle_events(unsigned max_msec, unsigned *p_count)
{
 ...
 * main().
 */
pj_status_t JSIceClient::icedemo_init(void)
{
    pj_status_t status;
    ...
    pool = pj_pool_create(&cp.factory, "icedemo",512, 512, NULL);
    
    /* Create timer heap for timer stuff */
    CHECK( pj_timer_heap_create(pool, 100, &ice_cfg.stun_cfg.timer_heap) );
    
    /* and create ioqueue for network I/O stuff */
    CHECK( pj_ioqueue_create(pool, 16, &ice_cfg.stun_cfg.ioqueue) );
    
    /* something must poll the timer heap and ioqueue,
     * unless we're on Symbian where the timer heap and ioqueue run
     * on themselves.
     */
    CHECK( pj_thread_create(pool, "icedemo", &icedemo_worker_thread,
                            this, 0, 0, &thread) );
...
void JSIceClient::reset_rem_info(void)
{
 ...
 */
int JSIceClient::encode_session(char buffer[], unsigned maxlen)
{
    char *p = buffer;
    unsigned comp;
    int printed;
    ...
    for (comp=0; comp<1/*opt.comp_cnt*/; ++comp) {
        unsigned j, cand_cnt;
...
void JSIceClient::icedemo_show_ice(void)
{
    ...
    else if (pj_ice_strans_sess_is_running(icest))
        puts("negotiation is in progress");
    else if (pj_ice_strans_has_sess(icest))
        puts("session ready");
    else
    ...
    printf("Role               : %s\n",
           pj_ice_strans_get_role(icest)==PJ_ICE_SESS_ROLE_CONTROLLED ?
    ...
    if (rem.cand_cnt==0) {
        ...
        for (i=0; i<rem.cand_cnt; ++i) {
            len = print_cand(buffer, sizeof(buffer), &rem.cand[i]);
            if (len < 0)
                err_exit("not enough buffer to show ICE status", -len);
 ...
 */
void JSIceClient::icedemo_help_menu(void)
{
    puts("");
 ...
 */
void JSIceClient::icedemo_print_menu(void)
{
    puts("");
    puts("+----------------------------------------------------------------------+");
    puts("|                    M E N U                                           |");
 ...
 * Main console loop.
 */
void JSIceClient::icedemo_console(void)
{
 ...
 */
void JSIceClient::icedemo_usage()
{
...
int JSIceClient::print_cand(char buffer[], unsigned maxlen,
...
// * And here's the main()
// */
//int iceDemo(int argc, char *argv[])
//{
